...
from provibench.bench.selection import UPTIME_FLOOR, Selection, SelectionDrop, SweepInfo
...
def render_candidates(
    selection: Selection, *, model: str, sort: str, zdr: bool, uptime_floor: float = UPTIME_FLOOR
) -> list[str]:
...
def selection_line(sweep: SweepInfo) -> str:
    ...
    sentences = _drop_sentences(
        (drop for drop in sweep.dropped if not drop.checked), sweep.uptime_floor
    )
    if sentences:
        line += " " + " ".join(sentences)
    return line
...
def not_probed_lines(sweep: SweepInfo) -> list[str]:
    ...
    """
    return _drop_sentences((drop for drop in sweep.dropped if drop.checked), sweep.uptime_floor)


def _drop_sentences(dropped: Iterable[SelectionDrop], floor: float) -> list[str]:
    ...
    for drop in dropped:
        groups.setdefault(_reason_words(drop, floor), []).append(_short_drop_label(drop))
    sentences: list[str] = []
...
def _reason_words(drop: SelectionDrop, floor: float) -> str:
    ...
    which floor this run actually used.
    """
    if drop.checked:
        return drop.reason
