...
def probe(  # noqa: PLR0913 (click binds one parameter per flag; there is no group to extract)
    ...
    *,
    ...
    estimates = [
        probe_estimate(spec, selected, options.rungs or [], options.repeats, prices.get(spec.label))
...
def _confirm(
    ...
    *,
    ...
    requests = len(specs) * sum(1 + count for count in options.repeats)
    question = (
...
def _progress_line(record: ProbeResult) -> str:
    message = (
        f"{record.spec_label} rung {record.rung} {record.role} {record.attempt} "
