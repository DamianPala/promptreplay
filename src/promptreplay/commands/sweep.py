...
from promptreplay.commands.probe import execute_probe, output_fields
from promptreplay.commands.probe_flags import ProbeRequest, probe_options
...
from promptreplay.commands.summary_view import SWEEP_BLOCK, render_probe_run
from promptreplay.commands.sweep_plan import (
    Criteria,
    endpoint_list,
    gateway_key,
    select,
...
from promptreplay.core.documents import Document, obj
...
def sweep(  # noqa: PLR0913 (click binds one parameter per flag; there is no group to extract)
    ...
    *,
    ...
    from promptreplay.bench.selection import pinned_spec
    from promptreplay.bench.selection_text import render_candidates
    ...
    if not specs:
        raise InvalidInput(
            ...
            hint="Pass --include TAG to keep a dropped endpoint, or check: promptreplay endpoints",
        )
