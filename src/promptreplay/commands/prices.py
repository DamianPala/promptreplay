...
from promptreplay.core.errors import OperationFailed
...
if TYPE_CHECKING:
    from promptreplay.bench.prices import Cached, PriceTable
...
@click.command(
    ...
    "cached under $XDG_CACHE_HOME/promptreplay/litellm-prices.json and refetched at most "
...
def prices(ctx: click.Context, *, update: bool, models: tuple[str, ...]) -> Document:
    ...
    from promptreplay.bench.prices import URL, TableUnavailable, format_age
    from promptreplay.commands.run_specs import load_targets
...
def cached_prices(invocation: Invocation, *, update: bool = False) -> Cached:
    ...
    table at all.
    """
    from promptreplay.bench.prices import cache_path, cached_table
...
def native_price_table(invocation: Invocation, specs: Sequence[RunSpec]) -> PriceTable | None:
    ...
    if not any(needs_litellm(spec) for spec in specs):
        return None
    from promptreplay.bench.prices import TableUnavailable
...
def _row(target: Target, model: str, table: PriceTable) -> Document:
    ...
    from promptreplay.bench.prices import resolve_target
