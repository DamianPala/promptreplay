...
`promptreplay.bench.targets` and `promptreplay.bench.trace` pull in pydantic; their symbols are
...
if TYPE_CHECKING:
    from promptreplay.bench.estimate import PreCheckCost, SpecEstimate, SpecPrices, UpperBound
...
def load_targets(invocation: Invocation) -> dict[str, Target]:
    ...
    invocation.message(
        ...
        "promptreplay config init copies them there"
    )
...
def parse_specs(raw_specs: Iterable[str], targets: dict[str, Target]) -> list[RunSpec]:
    ...
    from promptreplay.bench.targets import parse_run_spec
...
def fetch_model_endpoints(
    ...
    from promptreplay.bench.estimate import fetch_endpoint_index
...
def fetch_zdr_tags(model: str) -> set[str]:
    ...
    from promptreplay.bench.openrouter import fetch_zdr_endpoints
    ...
    *,
    ...
    if not kept:
        ...
        raise InvalidInput(
            ...
            hint="Drop a --exclude, or list the endpoints with: promptreplay endpoints MODEL",
        )
...
def validate_pinned_providers(
    ...
    for spec in run_specs:
        ...
        raise InvalidInput(
            ...
            hint="promptreplay endpoints MODEL",
        )
...
def spec_price_map(
    ...
    *,
    ...
    from promptreplay.bench.estimate import spec_prices
...
def check_budget(
    ...
    *,
    ...
    """
    from promptreplay.bench.estimate import estimate_total, same_amount
...
def select_conversation(
    ...
    from promptreplay.bench.trace import group_conversations, main_conversation
...
def _parse_targets(path: Path) -> dict[str, Target]:
    from promptreplay.bench.targets import load_targets
