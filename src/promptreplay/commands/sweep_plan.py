...
if TYPE_CHECKING:
    ...
    from promptreplay.bench.selection import Selection, SweepInfo
    from promptreplay.bench.targets import Target
...
def sweep_info(selection: Selection, criteria: Criteria) -> SweepInfo:
    ...
    """
    from promptreplay.bench.selection import SweepInfo, ranked
...
def select(endpoints: Sequence[Endpoint], criteria: Criteria) -> Selection:
    ...
    """
    from promptreplay.bench.selection import SelectionCriteria, select_candidates
...
def _reject_excluded_pins(criteria: Criteria) -> None:
    ...
    unanswerable next to the tag they can see in `promptreplay endpoints MODEL`.
    """
...
def endpoint_list(
    ...
    if not endpoints:
        raise OperationFailed(
            ...
            hint="Check the slug with: promptreplay endpoints MODEL",
        )
...
def gateway_key(sort: str, gateway: Target, invocation: Invocation) -> str | None:
    ...
    """
    from promptreplay.bench.selection import PERCENTILE_SORTS
    from promptreplay.bench.targets import resolve_api_key
...
def require_percentiles(endpoints: Sequence[Endpoint], sort: str, gateway: Target) -> None:
    ...
    """
    from promptreplay.bench.selection import PERCENTILE_SORTS, missing_percentiles
