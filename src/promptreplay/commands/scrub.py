...
`promptreplay.bench.*` pulls in pydantic; its symbols are imported only inside the callback,
...
from promptreplay.bench.scrub import SELECTION_ALL, SELECTION_CONVERSATION, SELECTION_ORDER
from promptreplay.commands.inspect import (
...
from promptreplay.core.errors import InvalidInput, PreconditionFailed
...
if TYPE_CHECKING:
    from promptreplay.bench.scrub import Replacement, ScrubReport
...
def scrub(  # noqa: PLR0913 (click binds one parameter per flag)
    ...
    *,
    ...
    from promptreplay.bench.trace import dump_document, write_trace_text
...
def _parse_replacement(spec: str) -> Replacement:
    ...
    from promptreplay.bench.scrub import Replacement
