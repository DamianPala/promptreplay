...
`promptreplay.bench.trace` pulls in pydantic; importing it only inside the callback keeps
...
are shared with `promptreplay.commands.replay` and `promptreplay.commands.scrub`.
"""
...
from promptreplay.core.errors import Action, InvalidInput, NotFound, OperationFailed
...
def inspect(ctx: click.Context, trace: str, conversation: str | None) -> Document:
    from promptreplay.bench.trace import group_conversations
...
def _select_key(
    ...
    from promptreplay.bench.trace import main_conversation
...
def resolve_trace_path(trace: str, invocation: Invocation) -> Path:
    ...
    from promptreplay.bench.trace import PackagedSampleMissing, TraceNotFound, resolve_trace
...
def load_trace_entries(trace_path: Path) -> list[TraceEntry]:
    ...
    from promptreplay.bench.trace import load_trace
...
def load_trace_text(trace_path: Path) -> str:
    ...
    from promptreplay.bench.trace import read_trace_text
...
def load_trace_documents(trace_path: Path) -> list[dict[str, Any]]:
    ...
    """
    from promptreplay.bench.trace import parse_trace_text, read_trace_text
...
def _read[T](reader: Callable[[], T], trace_path: Path) -> T:
    ...
    from promptreplay.bench.trace import TraceError
