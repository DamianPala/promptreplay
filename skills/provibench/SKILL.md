...
Install with `uv tool install provibench` or `pip install provibench`.
From a checkout, `uv sync`, then `uv run provibench`.
...
`provibench config show` prints the path of the targets file to edit for other targets or prices.
...
Run the same sweep on a schedule from a fixed directory, or with `PROVIBENCH_RUNS_DIR` set, so every run lands in one `runs_dir`.
