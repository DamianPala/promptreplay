...
| `first_hit_rate`, `first_cached_fraction`, `first_h` | the same three, pooled over only each rung's first warm read: one write then one read, the cold-start bound; equal to the unqualified fields with `--repeats 1`. `eff_per_m_prompt` is weighted by the pooled `h`, the steady state a long session runs in (prefix admitted, replicas warm); the gap to `first_h` is what the endpoint costs before that state is reached |
