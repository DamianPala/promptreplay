# provibench 0.2: first public release

Goal: publish provibench on GitHub and PyPI so that anyone running Claude Code or an OpenAI-compatible coding agent can measure, on their own recorded session, which provider actually caches their prompt and what it costs.
...
- Nonce `provibench-probe:<run hex>:<k>` prepended to the first system block; same across specs, different per rung, so every cold request is truly cold. `--warm` disables it. Nonce and run hex recorded in `run.json`.
...
- `provibench history MODEL [--trace T] [--since 30d]`: one row per run per provider with date, hit %, eff $/M, tok/s, errors, listed price; a sparkline per provider across runs; `--json` for scripts. `provibench compare RUN_A RUN_B` prints the delta per provider.
- README shows the cron line (`provibench sweep sample MODEL --yes --json >> sweeps.ndjson`); the default probe is already the cheap repeat, no separate `--quick`.
